...
void mlk_poly_tomont(mlk_poly *r)
{
  ...
  int ret = mlk_poly_tomont_native(r->coeffs);
  if (ret != -1)
  {
...
void mlk_poly_reduce(mlk_poly *r)
{
  ...
  int ret = mlk_poly_reduce_native(r->coeffs);
  if (ret != -1)
  {
...
void mlk_poly_mulcache_compute(mlk_poly_mulcache *x, const mlk_poly *a)
{
  ...
  int ret = mlk_poly_mulcache_compute_native(x->coeffs, a->coeffs);
  if (ret != -1)
  {
    return;
  }
...
void mlk_poly_ntt(mlk_poly *p)
{
  ...
  int ret = mlk_ntt_native(p->coeffs);
  if (ret != -1)
  {
...
void mlk_poly_invntt_tomont(mlk_poly *p)
{
  ...
  int ret = mlk_intt_native(p->coeffs);
  if (ret != -1)
  {
